...
export const parseBody = async (body: string) => {
  ...
  $('ul:not(ul ul)').each((_, element) => {
    $(element).addClass(
      'md:text-lg list-disc list-inside space-y-2 ml-2 pl-8 pr-4 py-6 indent-[-1em] border-dotted border-gray-500',
    )
  })
    ...
    if ($(element).text() === 'linkCard') {
      ...
      try {
        ...
        if (og['title'].match(/Amazon/)) {
          og['image'] = (ogData.result['ogImage'] as ImageObject[]).find((value) =>
